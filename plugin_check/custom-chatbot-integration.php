...
function ayd_chatbot_shortcode($atts) {
    ...
    if (!$has_permission) {
        return '<div class="ayd-error-message">
                    <p>You do not have permission to access this chatbot.</p>
    ...
    // If no permissions are set, allow access to all logged-in users
    if (empty($user_permissions)) {
        $has_permission = true;
    } else {
        // Check if user is in the permissions list
        $has_permission = in_array($current_user->ID, $user_permissions);
    }
    ...
    $iframe_url = sprintf(
        'https://julius.ai/embed/chatbot/%s?user=%s&email=%s&key=%s',
        urlencode($found_chatbot['id']),
        urlencode($current_user->display_name),
