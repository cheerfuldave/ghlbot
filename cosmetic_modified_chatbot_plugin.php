...
function chatbot_enqueue_styles() {
    wp_enqueue_style('chatbot-styles', plugins_url('css/chatbot-styles.css', __FILE__));
}
add_action('wp_enqueue_scripts', 'chatbot_enqueue_styles');

function test_chatbot_hooks() {
    echo '<div id="test-admin-head">Admin Head Hook Works</div>';
    echo '<div id="test-wp-head">WP Head Hook Works</div>';
}
add_action('admin_head', 'test_chatbot_hooks');
add_action('wp_head', 'test_chatbot_hooks');
